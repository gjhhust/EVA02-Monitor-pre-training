...
class HammingLoss(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        if outputs.shape != targets.shape:
            raise ValueError("Shape mismatch: outputs and targets must have the same shape")

            # 将概率值转换为二进制标签（0或1）
        binary_preds = torch.round(torch.sigmoid(outputs))
        # 计算Hamming Loss
        hamming_loss = ((binary_preds - targets) ** 2).mean()

        return hamming_loss
    

def get_args():
    parser = argparse.ArgumentParser('EVA fine-tuning and evaluation script for image classification', add_help=False)
    ...
    parser.add_argument('--muti_lables', action='store_true', default=False)
    parser.add_argument('--add_mode',default='save_pred', type=str, help='plot、save_pred')
    parser.add_argument('--add_mode_dir', default='/data/jiahaoguo/EVA/EVA-02/asuka/output/eva02_L_pt_m38m_p14_ft_30_448/show', type=str,
                        help='plot path')
    parser.add_argument('--label_dir', default='', type=str,
                        help='将imagedir替换成label_dir，后缀改成')
    parser.add_argument('--classes_json', default='', type=str,
                        help='分类映射')
    # robust evaluation
    ...
    # Model parameters
    parser.add_argument('--model', default='eva02_large_patch14_xattn_fusedLN_NaiveSwiGLU_subln_RoPE', 
                        type=str, metavar='MODEL', help='Name of model to train')

    parser.add_argument('--input_size', default=224, type=int,
                        help='images input size')
    parser.add_argument('--scale', default=[0.2, 1.0], type=float, nargs='+', metavar='BETA')
    ...
    parser.add_argument('--data_set', default='muti_label', choices=['CIFAR', 'IMNET', 'image_folder', "muti_label"],
...
def main(args, ds_init):
    ...
    if args.eval:
        if args.real_labels:
            ...
            test_stats = evaluate(data_loader_val, model, device, args.muti_lables, ds=args.enable_deepspeed, bf16=args.bf16, args=args)
            # print(f"Accuracy of the network on the {len(dataset_val)} test images: {test_stats['acc1']:.1f}%")
            print(f"Accuracy of the network on the {len(dataset_val)} test images: {test_stats}")
            exit(0)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if data_loader_val is not None:
            ...
            if args.model_ema and args.model_ema_eval:
                ...
                print(f"Accuracy of the EMA network on the {len(dataset_val)} test images: {ema_test_stats['acc1']:.1f}%")
